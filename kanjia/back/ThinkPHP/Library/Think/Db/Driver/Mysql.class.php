...
class Mysql extends Db{
    ...
    public function connect($config='',$linkNum=0,$force=false) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            //使用UTF8存取数据库
            _mysql_query("SET NAMES '".$config['charset']."'", $this->linkID[$linkNum]);
            //设置 sql_model
            if($dbVersion >'5.0.1'){
                _mysql_query("SET sql_mode=''",$this->linkID[$linkNum]);
            }
        }
    ...
    public function query($str) {
        ...
        // 记录开始执行时间
        G('queryStartTime');
        $this->queryID = _mysql_query($str, $this->_linkID);
        $this->debug();
    ...
    public function execute($str) {
        ...
        // 记录开始执行时间
        G('queryStartTime');
        $result =   _mysql_query($str, $this->_linkID) ;
        $this->debug();
        if ( false === $result) {
            $this->error();
            return false;
    ...
    public function startTrans() {
        ...
        if ($this->transTimes == 0) {
            _mysql_query('START TRANSACTION', $this->_linkID);
        }
        $this->transTimes++;
        return ;
    }
    ...
    public function commit() {
        if ($this->transTimes > 0) {
            $result = _mysql_query('COMMIT', $this->_linkID);
            $this->transTimes = 0;
            if(!$result){
                $this->error();
                return false;
    ...
    public function rollback() {
        if ($this->transTimes > 0) {
            $result = _mysql_query('ROLLBACK', $this->_linkID);
            $this->transTimes = 0;
            if(!$result){
                $this->error();
                return false;
